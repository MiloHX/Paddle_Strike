...
class ComputerAIP1 {
	...
	static public function update() {
		...
		if (system.game_type != CPU_VS_CPU)		return;

		var ball_h     = ball.object.transform.worldy();
		var ball_d     = ball.object.transform.worldx() - player_1.object.transform.worldx();
		var ball_dir   = ball.direction.x < 0;
		var player_1_h = player_1.object.transform.worldy();
		...
		if (!timer.isRunning()) {
			...
			if (action_flag) {
				// follow ball
				if (ball_h > player_1_h + 0.2) {
					if (ball_h - player_1_h > 0.2) {
						move(1);
					} else {
						if (Random.getFloatIn(0.0, 0.2) < (ball_h - player_1_h)) {
							move(1);
						}
					}
				} else if (ball_h < player_1_h - 0.2) {
					if (player_1_h - ball_h > 0.2) {
						move(-1);
					} else {
						if (Random.getFloatIn(0.0, 0.2) < (player_1_h - ball_h)) {
							move(-1);
						}
					}
				} 
